        ...
        {tab==='prof' && (
          <div className="kd-screen">
            ...
            <div className="kd-body" style={{ padding:16 }}>
              ...
              {/* สมัครเป็นไรเดอร์ — ส่งไปหน้าสมัครแพลตฟอร์มตัวจริง
                  ฟอร์มเดิมในหน้านี้เป็นของจำลอง กดส่งแล้วไม่มีใครได้รับใบสมัคร
                  ตัวจริงอยู่ที่ Rider Signup.html → บันทึกลง platform-worker และมีหน้าตรวจใน Back Office */}
              <a href="Rider%20Signup.html" className="kd-card" style={{ border:'none', cursor:'pointer', width:'100%', textAlign:'left', textDecoration:'none',
                ...
                <span style={{ width:40, height:40, borderRadius:12, background:'rgba(255,255,255,.15)', display:'flex', alignItems:'center', justifyContent:'center' }}>{React.cloneElement(IC.moto,{size:20})}</span>
                <div style={{ flex:1 }}><div style={{ fontWeight:700, fontSize:15 }}>{lang==='th'?'สมัครเป็นไรเดอร์ส่งอาหาร':'Become a delivery rider'}</div>
                  <div style={{ fontSize:12, opacity:.8, marginTop:2, lineHeight:1.45 }}>{lang==='th'?'สมัครกับแพลตฟอร์ม :Done KaiDee — รับงานจากทุกร้าน ไม่ใช่แค่ร้านนี้':'Join the :Done KaiDee platform — get jobs from every shop'}</div></div>
...
Object.assign(window, { RiderApp, JobCard, jobFromOrder });
